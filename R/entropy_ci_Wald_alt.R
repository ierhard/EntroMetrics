...
#'
#' @description
#' This function computes a confidence interval for Shannon entropy that relies on Basharin's asymptotic normality proof and an \eqn{O(1/n^2)} approximation of the variance of the maximum likelihood (ML) entropy estimator.
#'
...
#' entropy_ci_Wald_alt(c(4, 1, 5), z_score_dist = "t")$ci
#' entropy_ci_Wald_alt(c(4, 1, 5), z_score_dist = "normal")$ci
#'
#' # Using output from entropy_estimator_MM()
#' entropy_ci_Wald_alt(1:10)
#' entropy_ci_Wald_alt(1:10, pt_est_output = entropy_estimator_MM(1:10))


entropy_ci_Wald_alt <- function(

  bin_counts,
  conf_level = 0.95,
  z_score_dist = c("normal", "t"),
  pt_est = NULL,
  prob_est = NULL,
  ...
  eta_hat <- sum(p_hat[p_hat > 0] * (log2(p_hat[p_hat > 0]))^2)

  # Calculate confidence interval bounds

  ci_lower <- (H_hat - sqrt( H_hat^2 - (1 + z^2/n) * (H_hat^2 - z^2*eta_hat/n) )) / (1 + z^2/n)

  ci_upper <- (H_hat + sqrt( H_hat^2 - (1 + z^2/n) * (H_hat^2 - z^2*eta_hat/n) )) / (1 + z^2/n)
