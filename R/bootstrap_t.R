...
bootstrap_t <- function(x_vec,
  ...
  curve_data <- bootstrap_t_basic(x_vec = x_vec,
                                  pt_est_fct = pt_est_fct,
                                  conf_level = conf_level,
                                  B = B,
                                  B_sub = B_sub,
                                  se_fct = se_fct)[c("pt_boots", "se_boots")]
  ...
  spline_model <- lm(y ~ splines::bs(x, degree = 3, df = 10))
  ...
  g <- function(t){
    integrate(fct_to_integrate, lower = min(x), upper = t)$value
  }
