...
generate_batch_dt <- function(batch_size = 10^4) {
  ...
  N_vec <- floor(10^runif(batch_size, min = 0, max = 3)) + 4
  
  # Vector of alphabet sizes
  K_vec <- floor(10^runif(batch_size, min = 0, max = 4)) + 1
...
We now generate the data in batches (for memory efficiency), combine the batches, and save the data to disk.
...
We now calculate the features, response variables, and ancillary information.
